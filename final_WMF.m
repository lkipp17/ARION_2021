...
% copied from Monte_Carlo_WMF.m on 6/29/22
% 4 endmembers, Y-K 2008  method of determining SE endmember separately
...
% FINAL 2021 ARION data from ADC submissions (April 2023)
load ADC_radium.mat;
d18O = ADC_radium(:,10);
% salt = ADC_radium(:,8); missing a few samples- use bottle data for those
load salt.mat;
...
% load('/Users/Lauren/Dropbox/MATLAB/NABOS_2018/NABOS_2018_bot.mat');
% N = NO2+NO3;
% P = PO4;

% % NABOS 2018, using Channing's nutrient data from the surface + the salinity and d18O values collected with the Ra samples
% load('/Users/Lauren/Dropbox/MATLAB/NABOS_2018/WMF_data_subset.mat');
% % sample ID; sal; d18O; N; P
% salt = WMF_Ra_subset(:,2);
% d18O = WMF_Ra_subset(:,3);
% N = WMF_Ra_subset(:,4);
% P = WMF_Ra_subset(:,5);

% % GT 2015 *after running, use GT2015_match_Ra_WMF.m script to match with Ra data
...
% % 2015- both cruises
% % load('/Users/Lauren/Documents/MATLAB/Arctic GT/data organization for tpd paper/TPD_summary_final.mat');
...
% i = find(~isnan(TPD_sum(:,80))); %Ra-226

% % 2011 (RVDL 2018)- PS78
% load('/Users/Lauren/Dropbox/MATLAB/ARION_2021/PS78.mat');
% salt = PS78_salinity
% N = PS78_nitrate + PS78_nitrite;
% P = PS78_phosphate;
% d18O = PS78_d18O;
...
%        r_Pac(i,j) = abs(P(j) - P_atl)./(P_pac - P_atl);
       r_Pac(i,j) = (P_atl - P(j))./(P_atl - P_pac);
%        r_Atl(i,j) = 1-r_Pac(j);
   end
end
R_Pac = nanmean(r_Pac);
...
%% Discretely defined N:P relationship (instead of monte carlo)
% 
% % Jones 1998:
% % P_atl = (N + 3.072)./17.499;
% % P_pac = (N + 10.549)./12.368;
% 
% % Single equation - Whitmore 2020 & Newton GT
% for i = 1:n
%    for j = 1:length(N) 
%        P_atl = (N(j) + 3)./17;
%        P_pac = (N(j) + 11)./14;
%        r_Pac(i,j) = (P(j) - P_atl)./(P_pac - P_atl);
%    end
% end
% 
% % % Single equation - Y-K 2008
% % for i = 1:n
% %    for j = 1:length(N) 
% %        P_atl = (N(j) + 3.072)./17.499;
% %        P_pac = (N(j) + 11.306)./13.957;
% %        r_Pac(i,j) = (P(j) - P_atl)./(P_pac - P_atl);
% %    end
% % end
% 
% R_Pac = nanmean(r_Pac);
%% Monte carlo endmember values
...
WMF = [station sample output];
% WMF = [station sample d18O salt N P output];


fractions = [MW_avg'./100 SIM_avg'./100 ATL_avg'./100 PW_avg'./100]; % to copy/paste into master excel sheet